...
// Source: web/crypto.js, via scripts/sync-extension-libs.sh.
// Every top-level 'export ' is stripped (classic service worker script,
// loaded via importScripts() — no ES module system available), so each
// function below becomes an ambient global within the worker's scope.
// Re-run this script after any change to web/crypto.js.
...
async function deriveKeys(masterPassword, kdf) {
...
async function deriveRecoveryKeys(recoveryCodeBytes) {
...
async function wrapKeyBytes(rawKey, wrappingKey) {
  return aesGcmEncrypt(wrappingKey, rawKey);
}

async function unwrapKeyBytes(wrappedB64, wrappingKey) {
  return aesGcmDecrypt(wrappingKey, wrappedB64);
}

async function encryptJSON(obj, itemKey) {
  return aesGcmEncrypt(itemKey, enc.encode(JSON.stringify(obj)));
}

async function decryptJSON(blobB64, itemKey) {
...
async function encryptBytes(keyBytes, plaintextBytes) {
  const key = await crypto.subtle.importKey("raw", keyBytes, "AES-GCM", false, ["encrypt"]);
  const iv = randomBytes(12);
  const ct = new Uint8Array(await crypto.subtle.encrypt({ name: "AES-GCM", iv }, key, plaintextBytes));
  const out = new Uint8Array(iv.length + ct.length);
...
async function decryptBytes(keyBytes, ciphertextBytes) {
...
async function deriveSharedWrapKey(myPrivateKeyBytes, theirPublicKeyB64) {
...
async function deriveEmergencyAccessWrapKey(myPrivateKeyBytes, theirPublicKeyB64) {
...
async function derivePasskeyWrapKey(prfSecretBytes) {
